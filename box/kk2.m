function [leng,brea,heig,theta,centre,X] = kk2(X,cell_wid,thres,min_num,manual,nu_boxes_f_line,nu_pp_box)
 % Range of Cells
  % minimum values 
  [d,id]=min(X);
   a = X(id(1,1),:);
   ...
   endwhile
   x1=X;K1=K;
   X=del(K,nu_len,nu_bre,nu_pp_box);
     [d,id]=min(X);
   a = X(id(1,1),:);
   while (sum((sum(((X-ones(size(X,1),1)*a).^2),2)<(thres^2)))<min_num)
     X=[X(1:(id(1,1)-1),:);X((id(1,1)+1):size(X,1),:)];
...
function [p,J] = perp(x)
  ...
  function [x1,K1]=del(K,nu_len,nu_bre,nu_pp_box)
    x1=[];
    K1=K;
    for i=1:nu_len
      for j=1:nu_bre
        if (size(K1{i,j},1)<nu_pp_box)
          K1{i,j}=[];
        else x1=[x1;K1{i,j}];
        endif
      endfor
